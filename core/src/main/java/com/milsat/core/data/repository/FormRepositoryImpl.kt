...
internal class FormRepositoryImpl(
    ...
    override suspend fun createNewForm(config: String): Result<FormEntity> {
        return try {
            // Parse Json
            val configuration = configurationPasser.parseJson(jsonString = config) ?: return Result.Error(ErrorResponse.UnProcessableEntityError())


            if(configuration.forms.isEmpty()) return Result.Error(ErrorResponse.UnProcessableEntityError("This configuration file contains an empty form. Please select a valid one."))
